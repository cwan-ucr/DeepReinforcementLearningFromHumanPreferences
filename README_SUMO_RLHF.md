...
脚本默认会在网页里用 canvas 生成左右并排的道路视角实时动图，时间窗口固定为 10 秒，默认按 1.5 倍速度播放。ego 是蓝色车辆，背景车是灰色车辆；为了避免遮挡，背景车会分配到几条上下错开的视觉轨道。道路上方左侧显示速度/加速度随时间变化的小曲线，右侧显示完整 90 秒信号周期和当前相位滑动竖线；下方同时显示当前速度、加速度、前车距离和后车距离。
